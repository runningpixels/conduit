 ...
 *
 * The kinds split cleanly in two, and the split is the whole reason this
 ...
 *   - `code`, `text` and `image` are ordinary English words. Left as literals
 *     they sit untranslated in the middle of a translated sentence — "Dieses
 *     Dokument ist Code und…" — which is the exact seam a reader notices.
 *     German happens to borrow `code`/`text`, so that pair buys nothing there;
 *     French ("Texte") and Japanese ("テキスト", "画像") are why it is worth
 *     doing before waves 2 and 3 rather than after.
 *
